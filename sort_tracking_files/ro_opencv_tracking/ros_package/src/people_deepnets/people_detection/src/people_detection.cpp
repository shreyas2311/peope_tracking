	...
	// {
  	...
  	// 		ros::Publisher pos_array_pub_;
			...
			PeopleDetector::PeopleDetector(std::string name, String model_txt, String model_bin):it_(nh_),model_txt(model_txt), model_bin(model_bin)
			{
				...
				// local_nh.param("model_txt", model_txt_, string("../../models/MobileNetSSD_deploy.prototxt"));
				// local_nh.param("model_bin", model_bin_, string("../../models/MobileNetSSD_deploy.caffemodel" ));

				// model_txt = model_txt_;
				// model_bin = model_bin;

				// model_txt = nh_.resolveName("model_txt");
				// model_bin = nh_.resolveName("model_bin");
				// ROS_INFO("info is %s", model_txt_.c_str());
				// ROS_INFO("info2 is %s", model_bin_.c_str());
				...
				// ROS_INFO("info is %s", image_topic);
